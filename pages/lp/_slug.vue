...
export default {
  ...
  async fetch () {
    ...
    const brand = param.shift().toLowerCase()
    const model = param.shift()
    const sparePartSlug = param.join(' ').toLowerCase()
    ...
    if (this.search.brand && !this.models?.length) {
      await this.GET_MODELS(this.search)
    }
    const modelSlug = this.models.find(el => +el.modId === +model)
    if (modelSlug) {
